function [FRData,keyIdx]=processAll(groupBy,value,bins,rwdGrp)
...
for f=1:size(flist,1)
    ...
    fidx=fidx+1;
    flog(fidx)=f;
    futures(f)=parfeval(@processOne,3,flist,f,groupBy,value,bins);
...
for ffidx=1:fidx
    ...
    function [isEmpty,FRData,keyIdx]=processOne(flist,f,groupBy,value,bins)
        ...
        if ~isempty(evtTrials) && isfield(strSPK,'SPK') && ~isempty(strSPK.SPK)
            ...
            out=s2f.getAllFiringRate(s2f.processFile(evtTrials,strSPK.SPK),bins);
