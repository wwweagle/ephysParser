function selectivityChange(none,nogo,go)
resample=100;
    ...
    savefig(f,'selectivityChangeDNMS.fig');
    function out=getOne(data)
        b1=@(data,value) bootOne(data,value);
        out=nan(resample,4);
        out(:,1)=bootstrp(resample,b1,data(1:100,3:4),1);
        out(:,2)=bootstrp(resample,b1,data(101:end,3:4),-1);
        out(:,3)=bootstrp(resample,b1,data(1:100,6:9),1);
        out(:,4)=bootstrp(resample,b1,data(101:end,6:9),-1);        
...
%             sum(sum(data(1:100,6:9)==1))/sum(sum(data(1:100,6:9)~=0)),sum(sum(data(101:end,6:9)==-1))/sum(sum(data(101:end,6:9)~=0))];
    ...
    function out=bootOne(data,value)
        out=sum(sum(data==value))./sum(sum(data~=0));
    end
end