function svmCrossCluster(inFile)
...
%                         accuracyAll(:,:,ts)=svmOneTSBin(ts,instCount,allSpks,decRpt,c,g);
        ...
        for tIdx=1:2:size(accuracyAll,2)
            ...
            title(sprintf('%s, c@%.4f, g@%0.4f, n = %d',sprintf('%sDualCross %d ',inFile(1:end-4),tIdx),c,g,sum(cellfun(@(x) size(x,1),allSpks{1}))));
            print(sprintf('SVM%s_c%.4f_g%0.4f.png',sprintf('%sDualCross_%d_',inFile(1:end-4),tIdx),c,g),'-dpng');
%              close(fh);
...
%             pvShufAll((tIdx+1)/2,:)=pvShuf;
            ...
            pvShufAll(cIdx,gIdx,(tIdx+1)/2,:)=pvShuf;
        end
